...
public class EquipmentBorrowingController extends BaseController<EquipmentBorrowing, EquipmentBorrowingService> {
    ...
    public Map<String, Object> add(HttpServletRequest request) throws IOException {
        ...
        sql = ("SELECT count(*) count FROM `device_management` INNER JOIN `equipment_borrowing` ON device_management.equipment_number=equipment_borrowing.equipment_number WHERE device_management.inventory_quantity < equipment_borrowing.borrowed_quantity AND equipment_borrowing.equipment_borrowing_id="+max).replaceAll("&#60;","<");
        Integer count = service.selectBaseCount(sql);
        if(count>0){
            sql = "delete from "+"equipment_borrowing"+" WHERE "+"equipment_borrowing_id"+" ="+max;
