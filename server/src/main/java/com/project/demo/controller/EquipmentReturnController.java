...
public class EquipmentReturnController extends BaseController<EquipmentReturn, EquipmentReturnService> {
    ...
    public Map<String, Object> add(HttpServletRequest request) throws IOException {
        ...
        sql = ("SELECT count(*) count FROM `equipment_borrowing` INNER JOIN `equipment_return` ON equipment_borrowing.equipment_number=equipment_return.equipment_number WHERE equipment_borrowing.borrowed_quantity < equipment_return.return_quantity AND equipment_return.equipment_return_id="+max).replaceAll("&#60;","<");
        Integer count = service.selectBaseCount(sql);
        if(count>0){
            sql = "delete from "+"equipment_return"+" WHERE "+"equipment_return_id"+" ="+max;
            service.deleteBaseSql(sql);
            return error(30000,"归还数量超过借用数量！");
        }
